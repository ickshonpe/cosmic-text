...
impl SwashCache{
    ...
    pub fn get_image_uncached(&mut self, font_system: &FontSystem, cache_key: CacheKey) -> Option<SwashImage> {
        swash_image(font_system, &mut self.context, cache_key)
    }

    /// Create a swash Image from a cache key, caching results
    pub fn get_image(&mut self, font_system: &FontSystem, cache_key: CacheKey) -> &Option<SwashImage> {
        self.image_cache
            .entry(cache_key)
            .or_insert_with(|| swash_image(font_system, &mut self.context, cache_key))
    }

    pub fn get_outline_commands(&mut self, font_system: &FontSystem, cache_key: CacheKey) -> Option<&[swash::zeno::Command]> {
